...
#else
#define _HAS_MEMCPY_MEMMOVE_INTRINSICS 0
#endif
...
struct _Char_traits {
    ...
    _NODISCARD static _CONSTEXPR17 int compare(_In_reads_(_Count) const _Elem* _First1,
        _In_reads_(_Count) const _Elem* _First2, size_t _Count) noexcept {
    ...
    _NODISCARD static _CONSTEXPR17 size_t length(_In_z_ const _Elem* _First) noexcept {
        size_t _Count = 0;
        while (*_First != _Elem()) {
            ++_Count;
            ++_First;
    ...
    _NODISCARD static _CONSTEXPR17 const _Elem* find(
        _In_reads_(_Count) const _Elem* _First, size_t _Count, const _Elem& _Ch) noexcept {
...
public:
    ...
    _NODISCARD static _CONSTEXPR17 const _Elem* find(
        _In_reads_(_Count) const _Elem* _First, const size_t _Count, const _Elem& _Ch) noexcept {
...
public:
    ...
    _NODISCARD static _CONSTEXPR17 size_t length(_In_z_ const _Elem* const _First) noexcept {
...
public:
    ...
    _NODISCARD constexpr bool starts_with(const _Elem* const _Right) const noexcept {
    ...
    _NODISCARD constexpr bool ends_with(const _Elem* const _Right) const noexcept {
    ...
    _NODISCARD constexpr bool contains(const _Elem* const _Right) const noexcept {
        return find(_Right) != npos;
    }
#endif // _HAS_CXX23
...
public:
    ...
    _CONSTEXPR20 iterator erase(const const_iterator _Where) noexcept {
#if _ITERATOR_DEBUG_LEVEL != 0
        _STL_VERIFY(_Where._Getcont() == _STD addressof(_Mypair._Myval2), "string iterator incompatible");
#endif // _ITERATOR_DEBUG_LEVEL != 0
        const auto _Off = static_cast<size_type>(_Unfancy(_Where._Ptr) - _Mypair._Myval2._Myptr());
...
#pragma warning(pop)
    ...
    _CONSTEXPR20 void swap(basic_string& _Right) noexcept {
        if (this != _STD addressof(_Right)) {
...
private:
    ...
    _CONSTEXPR20 void _Eos(const size_type _New_size) {
    ...
    _CONSTEXPR20 void _Tidy_deallocate() noexcept {
        auto& _My_data = _Mypair._Myval2;
        _My_data._Orphan_all();
        _ASAN_STRING_REMOVE(*this);
        if (_My_data._Large_string_engaged()) {
...
#undef _CONSTEXPR20_STRING_LITERALS
